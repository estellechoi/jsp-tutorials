...
<%
	...
	if (r.equals("1")) {
		...
		if (request.getParameter("id_address") != null) {
	...
	String sql = "insert into ordered(username, email, product_code, size, qty, id_address, get_point, amount_buy, amount_discount, amount_pay, pay, pay_keychain, sender, confirm)";
	sql = sql + " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	PreparedStatement pstmt = conn.prepareStatement(sql);
